...
class SparseDelayMemoryPolynomial:
    def __init__(self, df, M, K, batch_size=64, learning_rate=0.001, epochs=10, hidden_layers=[64, 128], dropout_rate=0.5, l1_lambda=0.0, l2_lambda=0.0, patience=2, factor=0.9, edit_model=None, device=None, experiment_name=None):
        ...
        self.experiment_name = 'sparse_delay_memory_polynomial' if not experiment_name else experiment_name
        self.writer = self.initialize_log_dir(self.experiment_name)

        # Подготовка данных
        self.X, self.y, self.times = self.create_dataset(self.df, M, K, self.delays)
    ...
    @staticmethod
    def create_dataset(df, M, K, delays):
        """
        ...
        y_real = df['output_real'].values
        y_imag = df['output_imag'].values
        ...
        N = len(x_real)
        X = np.zeros((N, len(delays) * K * 2), dtype=np.float64)
        for n in range(M, N):
            index = 0
            for m in delays:
                if n - m >= 0:
