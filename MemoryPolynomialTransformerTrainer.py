...
class Transformer(nn.Module):
    def __init__(self, input_dim, model_dim, num_heads, num_layers, dropout_rate):
        ...
        self.fc_out = nn.Linear(model_dim, 2)  # Выходной слой для реальной и мнимой частей
...
class DefaultSimpleMLP(nn.Module):
    def __init__(self, input_dim, hidden_layers, output_dim=2, dropout_rate=0.5):
        super(DefaultSimpleMLP, self).__init__()
        layers = []
        in_dim = input_dim
...
class CombinedModel(nn.Module):
    def __init__(self, mlp, transformer):
        super().__init__()
        self.mlp = mlp
    ...
    def forward(self, x):
        mlp_out = self.mlp(x)
        transformer_out = self.transformer(x.unsqueeze(1))  # Добавляем дополнительное измерение для трансформера
        return mlp_out + transformer_out.squeeze(1)  # Суммируем выходы MLP и трансформера

class MemoryPolynomialTransformerTrainer:
    def __init__(self, df, M, K, batch_size=64, learning_rate=0.001, epochs=10, 
                 hidden_layers=[64, 128, 64], dropout_rate=0.5, l1_lambda=0.0, l2_lambda=0.0, 
                 patience=2, factor=0.9, device=None, experiment_name=None, 
                 num_heads=4, num_layers=2, model_dim=128):
        """
        Инициализация класса для тренировки модели Memory Polynomial с использованием MLP и трансформеров.
        ...
        Args:
            ...
            hidden_layers (list, optional): Конфигурация скрытых слоев MLP. По умолчанию [64, 128].
            dropout_rate (float, optional): Скорость дропаута. По умолчанию 0.5.
            ...
            num_heads (int, optional): Количество голов в трансформере. По умолчанию 4.
            num_layers (int, optional): Количество слоев трансформера. По умолчанию 2.
            model_dim (int, optional): Размерность модели трансформера. По умолчанию 128.
        """
        # Подготовка данных
        ...
        self.experiment_name = 'memory_polynomial_transformer' if not experiment_name else experiment_name
        ...
        # Инициализация MLP и трансформера
        mlp = DefaultSimpleMLP(input_dim=self.X.shape[1], hidden_layers=hidden_layers, output_dim=2, dropout_rate=dropout_rate).to(self.device)
        transformer = Transformer(input_dim=self.X.shape[1] + 1, model_dim=model_dim, 
                                  num_heads=num_heads, num_layers=num_layers, 
                                  dropout_rate=dropout_rate).to(self.device)

        self.model = CombinedModel(mlp, transformer).to(self.device)
    ...
    def train(self, max_early_stopping_counter=5):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, (times_batch, X_batch, y_batch) in enumerate(progress_bar):
                times_batch, X_batch, y_batch = times_batch.to(self.device), X_batch.to(self.device), y_batch.to(self.device)

                # Объединение временных меток с входными признаками
                X_with_times = torch.cat((X_batch, times_batch.unsqueeze(1)), dim=1)
                X_with_times = X_with_times[:, :-1]  # Удаляет последний признак
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for times_batch, X_batch, y_batch in self.train_loader:
                times_batch, X_batch, y_batch = times_batch.to(self.device), X_batch.to(self.device), y_batch.to(self.device)
