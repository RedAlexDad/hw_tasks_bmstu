...
def F(X_in, loading_model):
    with torch.no_grad():
        ...
        try:
            ...
            X_tensor = torch.tensor(X_scaled, dtype=torch.float32).unsqueeze(1).to(loading_model.device)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        model_path = "models/ctdrnn_20240929_124719.pth"
    else:        
    ...
    try:
        ...
        # Создаем двумерный массив истинных значений для реальной и мнимой частей
        Y_true = np.column_stack((df['output_real'].values, df['output_imag'].values))
        
        # Убедимся, что количество примеров совпадает
        Y_true = Y_true[max(model.p, model.q):]  # Обрезаем первые p или q значений

        rmse_real, rmse_imag = model.evaluate(Y_true, predictions)
