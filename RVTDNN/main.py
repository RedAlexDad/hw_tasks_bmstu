...
from RVTDNNModel import RVTDNNModel_0, RVTDNNModel_1, RVTDNNModel_2
...
def eval_batch_size(value):
    try:
        return eval(value)
    except:
        raise argparse.ArgumentTypeError(f"Batch size '{value}' is not a valid integer expression")
...
def argparse_setup():
    ...
    parser.add_argument('-t', '--time_delay', type=int, default=10, help='Time delay in samples')
    parser.add_argument('-b', '--batch_size', type=eval_batch_size, default=2**10, help='Batch size for training')
    ...
    parser.add_argument('-d', '--dropout_rate', type=float, default=0.25, help='Dropout rate for the network')
...
def main(args):
    ...
    try:
        # Создание и использование класса RVTDNNModel
        model = ModelTrainer(
        ...
        print(f'Evaluation RMSE (Real): {rmse_real:.8f}, RMSE (Imaginary): {rmse_imag:.8f}')
        model.logs_tensorboard.log_hparams_and_metrics(args.learning_rate, args.dropout_rate, args.batch_size, args.time_delay, args.epochs, rmse_real, rmse_imag)
        ...
        model.logs_tensorboard.log_hparams_and_metrics(args.learning_rate, args.dropout_rate, args.batch_size, args.time_delay, args.epochs, last_rmse_real, last_rmse_imag)
    finally:
        try:
            model.save_model_pt()
